...
@interface CustomVideoPlayerView ()
@property (nonatomic, strong) CIContext *ciContext; // Property to hold the CIContext
@end
...
- (void)updateVideoFrame {
    ...
    if ([self.playerItemOutput hasNewPixelBufferForItemTime:currentTime]) {
        ...
        if (pixelBuffer) {
            CGColorSpaceRef colorSpace = getColorSpaceFromPixelBuffer(pixelBuffer);

            if (colorSpace == NULL) {
                colorSpace = CGColorSpaceCreateDeviceRGB();
            }

            // Create CIContext only if it hasn't been created yet
            if (!self.ciContext) {
                self.ciContext = [CIContext contextWithOptions:@{ kCIContextWorkingColorSpace: (__bridge id)colorSpace }];
            }
